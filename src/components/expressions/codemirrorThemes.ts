...
export function useThemeFeature(
  ...
  effects.push(
    reconfigureMap.theme.reconfigure(theme.value === 'dark' ? darkTheme : whiteTheme),
  )
  if (theme.value === 'dark') {
    ...
    effects.push(
      reconfigureMap.highlight.reconfigure(syntaxHighlighting(darkHighlightStyle)),
    )
  }
  onMounted(() => {
    codeMirrorView.value?.dispatch({ effects })
  })
}
