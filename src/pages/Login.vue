    ...
    <div class="login-back flex items-center justify-center">
      <div class="login-code flex justify-center">
        <div v-if="!$route.params.token">
          <div
            v-if="!canLogin || $q.platform.within.iframe"
            class="login-card shadow-4 bg-white column items-center justify-center no-wrap"
          >
          ...
          >
            ...
            <div class="row full-width">
              <div class="col-12 text-center">
                <q-btn
                  @click="openWindow(`${authHost}/login/#/providers`)"
...
function openWindow(url: string, title = 'auth') {
  ...
  const newWindow = window.open(
    url,
    title,
    `toolbar=no,location=no,status=yes,resizable=yes,scrollbars=yes, width=${w}, height=${h}, top=${top}, left=${left}`,
  )
...
function manualLogin(
  callback: (loginModel: { token: string; region: TFlespiRegion }) => unknown,
) {
  const tokenHandler = (event: MessageEvent) => {
    if (typeof event.data === 'string' && ~event.data.indexOf('FlespiLogin|token:')) {
      ...
      let payload = event.data as string
